...
TraySetIcon("C:\Users\voidc\Desktop\journal.png")
...
taskCompletion := Map() 
weeklyEntry := Map()
for entry in StrSplit(IniRead("C:\Users\voidc\Desktop\themeSystemJournal.ini", selectedWeekName,, ""), "`n") {
...
defaultQuestions := Map("title", "`"Where are you today?`"", "box1", "`"What was good about your day?`"", "box2", "`"What was bad about your day?`"", "box3", "`"What are you thinking about?`"", "boxTask", "`"What's the most important task for the day?`"", "theme", "`"What's your theme for the year?`"", "description", "`"What does your theme mean to you?`"", "outcome", "`"What is one outcome you want as a part of your theme?`"")
dailyEntry := Map()
for entry in StrSplit(IniRead("C:\Users\voidc\Desktop\themeSystemJournal.ini", selectedDate,, ""), "`"`n", "`"") {
...
seasonalEntry := Map()
     for entry in StrSplit(IniRead("C:\Users\voidc\Desktop\themeSystemJournal.ini", selectedSeason " " selectedYear,, ""), "`"`n", "`"") {
        seasonalEntry[StrSplit(entry, "=")[1]] := StrSplit(entry, "=", "`"")[2]
    }
seasonalControlDataToWrite := ""

journalOpener.AddPicture("h30 w410 xm vseasonButton", "C:\Users\voidc\Desktop\" (HasVal(entries, selectedSeason " " selectedYear) ? StrLower(selectedSeason) : "no-entry") "-season.png")
...
journalOpener.AddPicture("h50 w410 xm Section vweekButton", "C:\Users\voidc\Desktop\" (HasVal(entries, selectedWeekName) ? "" : "no-entry-") "week.png")
...
for day in days {
    journalOpener.AddPicture("h50 w50 " (A_Index = 1 ? "xm" : "x+m") " ys+60 v" day, "C:\Users\voidc\Desktop\" (HasVal(entries, selectedWeek[7 - A_Index + 1]) ? "" : "no-entry-") StrLower(day) "-daily.png") ; window should be about 1450 px
...
changeDate(*) {
    ...
    journalOpener["seasonButton"].Value := "C:\Users\voidc\Desktop\" (HasVal(entries, selectedSeason " " selectedYear) ? StrLower(selectedSeason) : "no-entry") "-season.png"
    ...
    journalOpener["weekButton"].Value := "C:\Users\voidc\Desktop\" (HasVal(entries, selectedWeekName) ? "" : "no-entry-") "week.png"
    ...
    for day in days {
        journalOpener[day].Value := "C:\Users\voidc\Desktop\" (HasVal(entries, selectedWeek[7 - A_Index + 1]) ? "" : "no-entry-") StrLower(day) "-daily.png" 
    }
}
